# ==============================================================================
# SBL Campaign-Portable NetCDF Ingestion Engine (v2)
...
# Resolved JIT compiler type errors, missing dependency bugs, and JIT syntax warnings.
# ==============================================================================
...
using Dates
using LinearAlgebra
using Statistics # Fix 1: Required for mean() in looks_like_celsius

export ProfileData, IngestedCampaign, ingest_netcdf_gspt, try_extract_tower_2d, ingest_gabls3_netcdf

# Fix 4: Move const regex to module scope & make case-insensitive
const TOWER_REGEX = r"^((?:w_tc|u_w|v_w|hsb|usb|hlb|wth|wthv|ws|wd|tc|u|v|w|T|U|V))_?(\d+(?:\.\d+)?)m?$"i
...
function scrub_sentinels!(arr::AbstractArray)
    for i in eachindex(arr)
        ...
        elseif isnan(v)
            ...
            # Fix 3: Evaluate absolute value against positive sentinel magnitudes
            abs_v = abs(Float64(v))
            if abs_v == 999.0 || abs_v == 9999.0 || abs_v == 99999.0
                arr[i] = NaN
            end
        end
    end
...
function ingest_netcdf_gspt(filepath::String; z0 = 4.5e-4)
    ...
    Dataset(filepath, "r") do ds
        ...
        if !isnothing(tower_extracted)
            ...
            z_var = get_nc_var(ds, ["z", "height", "level", "nominal_height", "zf", "zt"])
            if isnothing(z_var)
        ...
        # 2. Safety Check: Enforce ascending monotonic vertical grids
...
# ------------------------------------------------------------------------------
# 5. GABLS3 SCM-Specific Dedicated Ingestion Entrypoint
# ------------------------------------------------------------------------------
...
function ingest_gabls3_netcdf(filepath::String)
    ...
    return NCDataset(filepath, "r") do ds
        ...
        return (z = z_coords, theta = theta_mat, U = U_mat, u = u_raw, v = v_raw, Nt = Nt, Nz = Nz)
    end
end
