...
# src/gspt_cases99_diagnostic.jl
...
function evaluate_spline(x_knot::Vector{Float64}, a::Vector{Float64}, M::Vector{Float64}, h::Vector{Float64}, x_eval::Float64; order::Int=0)
    ...
    # Find the active interval in logarithmic time.
...
"""
function calculate_gspt_diagnostics(p::CASES99Params)
    # 1. Simulate profiles
    ...
    for i_t in 1:n_t
        ...
        # Using s=0.08 smoothing factor to suppress high-frequency spline wobbles
        ...
        # Eliminates the need for secondary spline fitting on zeta
        zeta = zeros(n_fine)
        zeta_z = zeros(n_fine)
        ...
        @views C_const_matrix[:, i_t] .= C_const
        @views C_coord_matrix[:, i_t] .= C_mapping

        # Local adaptive curvature scale (K_0) with a safety floor
        K_0 = median(abs.(Ri_g_zz)) + 1e-6

        # Mapping-curvature fraction (C_M)
        @views @. CM_matrix[:, i_t] = abs(C_mapping) / (abs(C_const) + abs(C_mapping) + p.epsilon_c * K_0)
    end

    # Export results as a standard flat trajectory CSV for downstream analysis
    export_gspt_results(z_fine, hours, CM_matrix, Ri_matrix, z_llj, h_inv)
