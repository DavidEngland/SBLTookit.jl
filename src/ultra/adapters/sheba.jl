...
function extract_sheba_profiles(
    ...
    # Fix: Correctly check Symbols against DataFrame properties
    missing_cols = filter(c -> !hasproperty(df, c), [lo_col, hi_col])
    isempty(missing_cols) || error("Input DataFrame missing columns: $(missing_cols)")
...
function extract_sheba_observations(
    ...
    for prof in profiles
        ...
        push!(observations, StandardizedBLObservation(
            ...
            z0m,
            false, # Correctly flagged: N=2 is insufficient for eta_3 (requires N >= 4)
            n_valid,
        ))
    end
...
@inline function _row_float(row, col::Symbol, default::Float64)
    hasproperty(row, col) || return default
