...
function run_campaign_gspt_transition(nc_path::String, campaign_name::String)
    ...
    # Ingest multi-level NetCDF & process nocturnal profiles
    results = ingest_netcdf_gspt(nc_path; S2_min=1e-3, σ_u=0.05, σ_v=0.05, σ_th=0.05)

    # Render heatmap & save plot
...
# Dynamic campaign file discovery supporting symlinks and nested paths
...
for campaign in campaigns
    ...
    # Recursively find all .nc files across directory hierarchy
    nc_files = String[]
    try
        ...
        @warn "Could not read directory symlink target for $(campaign): $(e)"
    end
