# ==============================================================================
# SBL ASFG 3.0 Ingestion and Stage 1-6 Processing Pipeline
...
function create_spline_workspace(xi::Vector{Float64})
    N = length(xi)
    h = diff(xi)
...
function extract_spline_derivatives(ws::SplineWorkspace, profile::Vector{Float64}, z::Vector{Float64})
    N = ws.N
...
function process_asfg_timestep(
    ...
    sigma_theta::Vector{Float64},
    sigma_U::Vector{Float64},
...
function parse_asfg_line(line::String, header_map::Dict{String, Int})
    parts = split(line)
    vals = parse.(Float64, parts)
    
    # Clean helper to parse column safely or fallback
    get_col(name, default=0.0) = haskey(header_map, lowercase(name)) ? vals[header_map[lowercase(name)]] : default
    
    time_val = get_col("time", get_col("hour", 0.0))
    tsfc = get_col("Tsfc", 270.0)
    ...
    # 30-min data count baseline at 1Hz is 1800 samples
    ...
    theta = (T .+ 273.15) .+ 0.0098 .* z
    
    # Dynamic noise-floor construction
    sigma_theta = zeros(5)
    sigma_U = zeros(5)
    
    for i in 1:5
        if fl[i] == 1.0
            # DEFENSE: Corrupt Level -> Inflate variance to 10^6 (sigma = 10^3) to bypass
            sigma_theta[i] = 1000.0
            sigma_U[i] = 1000.0
        else
            # Valid sensor level -> compute standard in-situ noise-floor scales
            sigma_theta[i] = max(sqrt(sgT[i] / No[i]), 0.02)
            sigma_U[i] = max(sqrt((sgu[i] + sgv[i]) / No[i]), 0.01)
        end
    end
    ...
    if use_threads
        Threads.@threads for t in 1:M
            try
                ...
                # NOCTURNAL TRIGGER: w'theta' = hs / 1200 K m/s. Only negative sensible heat fluxes processed.
...
println("=====================================================================================")
println("SBL ASFG 3.0 INGESTION & STAGE 4 COVARIANCE INFLATION AUDIT")
println("=====================================================================================")
